...
while cv.waitKey(1) < 0:
    ...
    if has_frame:
        ...
        ### print real-time FPS value
        if (time.time() - start_time) > fps_interval:
            ### calculate frame. FPS = interval 1sec
            realtime_fps = fps_count / (time.time() - start_time)
            fps_count = 0  # few frame
            start_time = time.time()
        ...
        ### print the current runtime and the total frame num
        if frame_count == 1:
            run_timer = time.time()
